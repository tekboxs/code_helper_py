...
class LoggingFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        formatter = logging.Formatter(
            current_format, "%Y-%m-%d %H:%M:%S", style="{")
        current_format = "(black){asctime}(reset) (levelcolor){levelname:<8}(reset) (green){name}(reset) {message}"
        current_format = current_format.replace(
            "(black)", self.black + self.bold)
        current_format = current_format.replace("(reset)", self.reset)
...
mencion_cooldown = commands.CooldownMapping.from_cooldown(
    1, 60, commands.BucketType.user)
error_cooldown = commands.CooldownMapping.from_cooldown(
...
class DiscordBot(commands.Bot):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('https://api.quotable.io/random') as response:
                if response.status == 200:
                    ...
                    tradutor = Translator()
                    conteudo_traduzido = tradutor.translate(
                        quote_data['content'], dest='pt').text
                    autor_traduzido = tradutor.translate(
                        quote_data['author'], dest='pt').text
    ...
    async def setup_hook(self) -> None:
        ...
        cred = credentials.Certificate(
            f"{os.path.realpath(os.path.dirname(__file__))}/firebase/key.json")
    ...
    async def on_command_error(self, context: Context, error) -> None:
        ...
        retried = error_cooldown.get_bucket(
            context.message).update_rate_limit()
        if retried:
            current_attemps = 0
