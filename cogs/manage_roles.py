...
class ManageRolesCog(commands.Cog):
    ...
    @classmethod
    def get_roles_options(cls, guild):
        roles_list = [{"label": role.name, "value": role.id} for role in guild.roles]
        return roles_list

    @classmethod
    async def delete_role(cls, interaction: discord.Interaction, role_id: int):
        role = discord.utils.get(interaction.guild.roles, id=role_id)
        await role.delete()
        await interaction.channel.send(f'{role.name} foi jogar no vasco')
    ...
    async def list_roles(self, ctx: Context):
        ...
        for item in roles_options:
            ...
            await ctx.channel.send(content=message_content,
                                   view=GenericButtonView(self.delete_role(role_id=item['value']), 'Deletar'))
            i += 1
            if i == 4:
                break
