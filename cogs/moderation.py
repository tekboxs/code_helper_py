...
class Moderacao(commands.Cog, name="moderacao"):
    ...
    @commands.hybrid_command(name="expulsar", description="Expulsa um usuário do servidor.")
    @commands.has_permissions(kick_members=True)
    @commands.bot_has_permissions(kick_members=True)
    @app_commands.describe(user="O usuário que deve ser expulso.", reason="A razão pela qual o usuário deve ser expulso.")
    async def expulsar(self, context: Context, user: discord.User, *, reason: str = "Não especificado") -> None:
        # Comando para expulsar um usuário do servidor.
        member = context.guild.get_member(user.id) or await context.guild.fetch_member(user.id)
        if member.guild_permissions.administrator:
            await context.send(embed=discord.Embed(description="O usuário tem permissões de administrador.", color=0xE02B2B))
        else:
            try:
                await context.send(embed=discord.Embed(description=f"**{member}** foi expulso por **{context.author}**!", color=0xBEBEFE).add_field(name="Razão:", value=reason))
                await member.send(f"Você foi expulso por **{context.author}** de **{context.guild.name}**!\nRazão: {reason}")
                await member.kick(reason=reason)
            except:
                await context.send(embed=discord.Embed(description="Ocorreu um erro ao tentar expulsar o usuário. Certifique-se de que meu cargo está acima do cargo do usuário que você quer expulsar.", color=0xE02B2B))

    @commands.hybrid_command(name="apelido", description="Altera o apelido de um usuário no servidor.")
    @commands.has_permissions(manage_nicknames=True)
    @commands.bot_has_permissions(manage_nicknames=True)
    @app_commands.describe(user="O usuário que deve ter um novo apelido.", nickname="O novo apelido que deve ser definido.")
    async def apelido(self, context: Context, user: discord.User, *, nickname: str = None) -> None:
        ...
        try:
            ...
            await context.send(embed=discord.Embed(description=f"O novo apelido de **{member}** é **{nickname}**!", color=0xBEBEFE))
        except:
            await context.send(embed=discord.Embed(description="Ocorreu um erro ao tentar alterar o apelido do usuário. Certifique-se de que meu cargo está acima do cargo do usuário que você quer alterar o apelido.", color=0xE02B2B))
    ...
    async def banir(self, context: Context, user: discord.User, *, reason: str = "Não especificado") -> None:
        ...
        try:
            if member.guild_permissions.administrator:
                ...
                await context.send(embed=discord.Embed(description=f"**{member}** foi banido por **{context.author}**!", color=0xBEBEFE).add_field(name="Razão:", value=reason))
                await member.send(f"Você foi banido por **{context.author}** de **{context.guild.name}**!\nRazão: {reason}")
                await member.ban(reason=reason)
        except:
            await context.send(embed=discord.Embed(title="Erro!", description="Ocorreu um erro ao tentar banir o usuário. Certifique-se de que o ID é um ID existente que pertence a um usuário.", color=0xE02B2B))
    ...
    async def advertencia(self, context: Context) -> None:
        ...
        if context.invoked_subcommand is None:
            await context.send(embed=discord.Embed(description="Especifique um subcomando.\n\n**Subcomandos:**\n`adicionar` - Adiciona uma advertência a um usuário.\n`remover` - Remove uma advertência de um usuário.\n`listar` - Lista todas as advertências de um usuário.", color=0xE02B2B))

    @advertencia.command(name="adicionar", description="Adiciona uma advertência a um usuário no servidor.")
    @commands.has_permissions(manage_messages=True)
    @app_commands.describe(user="O usuário que deve ser advertido.", reason="A razão pela qual o usuário deve ser advertido.")
    async def advertencia_adicionar(self, context: Context, user: discord.User, *, reason: str = "Não especificado") -> None:
        ...
        await context.send(embed=discord.Embed(description=f"**{member}** foi advertido por **{context.author}**!\nTotal de advertências: {total}", color=0xBEBEFE).add_field(name="Motivo:", value=reason))
        try:
    ...
    @commands.has_permissions(manage_messages=True)
    @app_commands.describe(user="O usuário que deve ter sua advertência removida.", warn_id="O ID da advertência que deve ser removida.")
    async def advertencia_remover(self, context: Context, user: discord.User, warn_id: int) -> None:
        ...
        await context.send(embed=discord.Embed(description=f"Removi a advertência **#{warn_id}** de **{member}**!\nTotal de advertências: {total}", color=0xBEBEFE))
    ...
    async def advertencia_listar(self, context: Context, user: discord.User) -> None:
        ...
        description = "Esse usuário não tem advertências." if len(warnings_list) == 0 else "".join(f"• Advertido por <@{warning[2]}>: **{warning[3]}** (<t:{warning[4]}>) - Warn ID #{warning[5]}\n" for warning in warnings_list)
    ...
    async def limpar(self, context: Context, amount: int) -> None:
        ...
        await context.channel.send(embed=discord.Embed(description=f"**{context.author}** deletou **{amount}** mensagens!", color=0xBEBEFE))

    @commands.hybrid_command(name="banirremoto", description="Bane um usuário sem que ele precise estar no servidor.")
    @commands.has_permissions(ban_members=True)
    @commands.bot_has_permissions(ban_members=True)
    @app_commands.describe(user_id="O ID do usuário que deve ser banido.", reason="A razão pela qual o usuário deve ser banido.")
    async def banirremoto(self, context: Context, user_id: str, *, reason: str = "Não especificado") -> None:
        # Comando para banir um usuário remotamente.
        try:
            await self.bot.http.ban(user_id, context.guild.id, reason=reason)
            user = self.bot.get_user(int(user_id)) or await self.bot.fetch_user(int(user_id))
            await context.send(embed=discord.Embed(description=f"**{user}** (ID: {user_id}) foi banido por **{context.author}**!", color=0xBEBEFE).add_field(name="Razão:", value=reason))
        except Exception:
            await context.send(embed=discord.Embed(description="Ocorreu um erro ao tentar banir o usuário. Certifique-se de que o ID é um ID existente que pertence a um usuário.", color=0xE02B2B))

    @commands.hybrid_command(name="arquivar", description="Arquiva em um arquivo de texto as últimas mensagens com um limite escolhido de mensagens.")
    @commands.has_permissions(manage_messages=True)
    ...
    async def arquivar(self, context: Context, limit: int = 10) -> None:
        ...
        with open(log_file, "w", encoding="UTF-8") as file:
            file.write(f'Arquivo de mensagens do: #{context.channel} ({context.channel.id}) no servidor "{context.guild}" ({context.guild.id}) em {datetime.now().strftime("%d/%m/%Y %H:%M:%S")}\n')
            async for message in context.channel.history(limit=limit, before=context.message):
                ...
                file.write(f"{message.created_at.strftime('%d/%m/%Y %H:%M:%S')} {message.author} {message.id}: {message.clean_content} {attachments_text}\n")
...
#         try:
#             await member.send(
#                 f"Você levou uma adivertência em **{context.guild.name}**!\nMotivo: {reason}"
#             )
#         except:
#             # Couldn't send a message in the private messages of the user
#             await context.send(
#                 f"{member.mention}, esteja avisado por **{context.author}**!\nMotivo: {reason}"
#             )
...
#             description = "Esse usuário não tem advertências."
#         else:
#             for warning in warnings_list:
